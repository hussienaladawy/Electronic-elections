            ...
            <form action="{{ route("super_admin.admins.update", $admin->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error("permissions") is-invalid @enderror" id="permissions" name="permissions[]" multiple>
                        <option value="manage_users" {{ in_array("manage_users", old("permissions", $admin->permissions ?? [])) ? "selected" : "" }}>إدارة المستخدمين</option>
                        <option value="manage_elections" {{ in_array("manage_elections", old("permissions", $admin->permissions ?? [])) ? "selected" : "" }}>إدارة الانتخابات</option>
                        <option value="view_reports" {{ in_array("view_reports", old("permissions", $admin->permissions ?? [])) ? "selected" : "" }}>عرض التقارير</option>
                    </select>
                    @error("permissions")
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
