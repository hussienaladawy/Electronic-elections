    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="btn-group">
                    <a href="{{ route('super_admin.notifications.markAllAsRead') }}" class="btn btn-sm btn-primary">تمييز الكل كمقروء</a>
                    <form action="{{ route('super_admin.notifications.deleteAllRead') }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف جميع الإشعارات المقروءة؟');">
                        @csrf
                    ...
                    <div class="alert alert-light border-start border-4 {{ $notification->read_at ? 'border-secondary' : 'border-primary' }} p-3 mb-2 d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            ...
                            @unless($notification->read_at)
                                <a href="{{ route('super_admin.notifications.markAsRead', $notification->id) }}" class="btn btn-sm btn-success ms-2">تمييز كمقروء</a>
                            @endunless
                            <form action="{{ route('super_admin.notifications.destroy', $notification->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذا الإشعار؟');" class="d-inline">
                                @csrf
                                @method('DELETE')
