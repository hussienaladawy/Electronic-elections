    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        <a href="{{ route('admin.assistants.edit', $assistant->id ?? 1) }}" class="btn btn-warning">
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        <a href="{{ route('admin.assistants.index') }}" class="btn btn-outline-primary">
                            <i class="fas fa-list mr-1"></i>
                            قائمة المساعدين
                        </a>
                        <a href="{{ route('admin.assistants.create') }}" class="btn btn-outline-success">
...
function toggleStatus(assistantId, status) {
    ...
    if (confirm(`هل أنت متأكد من ${action} هذا المساعد؟`)) {
        fetch(`{{ route('admin.assistants.toggle-status', '') }}/${assistantId}`, {
            method: 'POST',
            headers: {
...
function sendPasswordReset(assistantId) {
    if (confirm('هل تريد إرسال رابط إعادة تعيين كلمة المرور للمساعد؟')) {
        fetch(`{{ route('admin.assistants.send-password-reset', '') }}/${assistantId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function sendMessage(assistantId) {
    ...
    if (message && message.trim()) {
        fetch(`{{ route('admin.assistants.send-message', '') }}/${assistantId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function deleteAssistant(assistantId) {
    if (confirm('هل أنت متأكد من حذف هذا المساعد؟ هذا الإجراء لا يمكن التراجع عنه.')) {
        fetch(`{{ route('admin.assistants.destroy', '') }}/${assistantId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        .then(data => {
            if (data.success) {
                window.location.href = '{{ route("admin.assistants.index") }}';
            } else {
