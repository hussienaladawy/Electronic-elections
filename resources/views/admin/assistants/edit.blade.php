    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.assistants.update', $assistant->id ?? 1) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <div>
                                <a href="{{ route('admin.assistants.show', $assistant->id ?? 1) }}" class="btn btn-info">
...
function sendWelcomeEmail(assistantId) {
    if (confirm('هل تريد إرسال بريد ترحيبي للمساعد؟')) {
        fetch(`{{ route('admin.assistants.send-welcome-email', '') }}/${assistantId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
