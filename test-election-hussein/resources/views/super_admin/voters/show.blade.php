    ...
    <div class="row">
        ...
        <div class="col-lg-4">
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        <a href="{{ route("super_admin.voters.edit", $voter->id ?? 1) }}" class="btn btn-warning">
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        <a href="{{ route("super_admin.voters.index") }}" class="btn btn-outline-primary">
                            <i class="fas fa-list mr-1"></i>
                            قائمة الناخبين
                        </a>
                        <a href="{{ route("super_admin.voters.create") }}" class="btn btn-outline-success">
...
function toggleStatus(voterId, status) {
    ...
    if (confirm(`هل أنت متأكد من ${action} هذا الناخب؟`)) {
        fetch(`{{ route("super_admin.voters.toggle-status", "") }}/${voterId}`, {
            method: "POST",
            headers: {
...
function sendPasswordReset(voterId) {
    if (confirm("هل تريد إرسال رابط إعادة تعيين كلمة المرور للناخب؟")) {
        fetch(`{{ route("super_admin.voters.send-password-reset", "") }}/${voterId}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-CSRF-TOKEN": document.querySelector("meta[name="csrf-token"]").getAttribute("content")
...
function sendMessage(voterId) {
    ...
    if (message && message.trim()) {
        fetch(`{{ route("super_admin.voters.send-message", "") }}/${voterId}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-CSRF-TOKEN": document.querySelector("meta[name="csrf-token"]").getAttribute("content")
...
function deleteVoter(voterId) {
    if (confirm("هل أنت متأكد من حذف هذا الناخب؟ هذا الإجراء لا يمكن التراجع عنه.")) {
        ...
        .then(data => {
            if (data.success) {
                window.location.href = "{{ route("super_admin.voters.index") }}";
            } else {
