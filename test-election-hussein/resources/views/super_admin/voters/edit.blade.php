    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route("super_admin.voters.update", $voter->id ?? 1) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method("PUT")
...
<div class="mb-3">
    ...
    @enderror
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <div>
                                <a href="{{ route("super_admin.voters.show", $voter->id ?? 1) }}" class="btn btn-info">
...
function sendWelcomeEmail(voterId) {
    if (confirm("هل تريد إرسال بريد ترحيبي للناخب؟")) {
        fetch(`{{ route("super_admin.voters.send-welcome-email", "") }}/${voterId}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-CSRF-TOKEN": document.querySelector("meta[name="csrf-token"]").getAttribute("content")
